...
`build-master-brain` now runs without the live progress UI by default to reduce terminal popup noise. Add `--show-progress` when you explicitly want the live progress view.

The default Master Brain template now includes a Humanities group (`6_Humanities_Brain`) with Philosophy, History, Literature, Linguistics, Arts, and Religion subfolders.

Module routing now reads aliases from module config (for example, `config/master_brain.toml`) so cross-disciplinary queries can match configured brain aliases directly.
